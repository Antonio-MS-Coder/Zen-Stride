...
class IgniteDataStore: ObservableObject {
    ...
    // UserDefaults keys
    private let habitsKey = "ignite.habits"
    private let winsKey = "ignite.wins"
    private let tokensKey = "ignite.tokens"
    
    init() {
        loadFromUserDefaults()
        
        // Load default habits if this is the first launch
        if habits.isEmpty {
            loadDefaultHabits()
        }
    ...
    func deleteWin(_ win: MicroWin) {
        wins.removeAll { $0.id == win.id }
        calculateStreak()
        saveToUserDefaults()
    }
    
    func updateWin(_ win: MicroWin) {
